                ...
                if (!ignorenonamesp || e.ObjectName.Length != 0)
                {
                    if (transform)
                    {
                        AddEvent(cnt, e.DatabaseID, e.DatabaseName
                            ...
                            , e.ObjectName.Length == 0 ? lex.StandardSql(e.TextData) : e.TextData, e.CPU, e.Reads,
                            e.Writes, e.Duration, e.Count, e.RowCounts);
                    }
                    else
                    {
                        AddEvent(cnt, e.DatabaseID, e.DatabaseName, e.ObjectID, e.ObjectName, e.TextData, e.CPU,
                            e.Reads, e.Writes, e.Duration, e.Count, e.RowCounts);
                    }
                }
            }
